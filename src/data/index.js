...
function normalizeSpec(spec, make = '', model = '', trim = '') {
  ...
  return {
    ...
    hp: spec.power ? spec.power + ' kW' : '—',
    tq: spec.torque ? spec.torque + ' Nm' : '—',
    tx: spec.transmission || '—',
    dr,
    z1: spec.acceleration ? `~${spec.acceleration} sec` : '—',
    qm: spec.quarterMile || '—',
    ts: isEV ? (spec.range ? spec.range + ' km (WLTP)' : '—') : (spec.topSpeed ? spec.topSpeed + ' km/h' : '—'),
    fc: fmtL(spec.fc),
    fh: fmtL(spec.fh),
    fx: fmtL(spec.fx),
